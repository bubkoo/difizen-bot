...
import { PullRequestContext, State } from './types'
import { getCommitSubjects, getMatcher, getOutput } from './util'

export async function getStatus(context: PullRequestContext): Promise<State> {
  ...
  const { configs, manual } = await getConfig(context)
  ...
  for (let i = 0; i < configs.length; i += 1) {
    ...
    if (match == null) {
      if (commitSubjects == null) {
        commitSubjects = (await getCommitSubjects(context)) || []
      }
...
export async function hasChange(context: PullRequestContext, nextState: State) {
  ...
  const checkRuns = data.check_runs.filter(
    item =>
      item.external_id != null && item.external_id.startsWith(`[${checkName}]`),
  )

  if (checkRuns.length === 0) {
    context.log.info('[wip] No previous check runs.')
    return true
  }

  context.log.info(`[wip] Found check runs: ${checkRuns.length}`)
...
export async function updateStatus(
  ...
  const output = getOutput(context, nextState)

  context.log.info(`[wip] Create check run.`)
  context.log.info(`  metadata: ${JSON.stringify(options)}`)
  context.log.info(`  output: ${JSON.stringify(output)}`)
